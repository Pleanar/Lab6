...
public class HumanBeingModule implements Module<HumanBeing> {
    ...
    public HumanBeing buildObject() throws BuildObjectException {
        try {
            ...
            outputModule.writeByDefault("Enter the value ID (Type: Integer). This field cannot be skipped.");
            String ID = scanner.next();
            while (true){
                Long id = Utilities.handleUserInputID(ID);
                if (id != null){
                    result.setId(id);
                    break;
                } else {
                outputModule.writeException("Value violates restrictions for field! Try again.");
                outputModule.writeInfo("Restrictions: Should be Integer.");
                }
                ID = scanner.next();
            }
            ...
            while (true){
                ...
                try {
                    weapon = WeaponType.valueOf(answer.toUpperCase());
            ...
            while (true){
                ...
                Mood mood = null;
                try {
                    mood = Mood.valueOf(answer.toUpperCase());
                } catch (IllegalArgumentException ignored) {}
                if (mood != null){
                    result.setMood(mood);
                    break;
